function s2FineGridXY(gridG, x0, y0, x1, y1, vCount, hCount) {
    ...
    for (i = 1; i < vCount; i++) {
      ...
      gridG.append("line").attr("x1", vx).attr("x2", vx).attr("y1", y0).attr("y2", y1)
        .attr("stroke", i % 5 === 0 ? sm : sf).attr("stroke-width", i % 5 === 0 ? wm : wf).attr("stroke-dasharray", i % 5 === 0 ? "none" : "2 5");
    }
    for (i = 1; i < hCount; i++) {
      hy = y0 + (i / hCount) * (y1 - y0);
      gridG.append("line").attr("x1", x0).attr("x2", x1).attr("y1", hy).attr("y2", hy)
        .attr("stroke", i % 4 === 0 ? sm : sf).attr("stroke-width", i % 4 === 0 ? wm : wf).attr("stroke-dasharray", i % 4 === 0 ? "none" : "2 5");
    }
  }
  function s2PolarFineRays(gridG, cx, cy, R, nRays) {
    ...
    for (k = 0; k < nRays; k++) {
      ...
      gridG.append("line").attr("x1", cx).attr("y1", cy).attr("x2", cx + Math.cos(ang) * R).attr("y2", cy + Math.sin(ang) * R)
        .attr("stroke", k % 3 === 0 ? sm : sf).attr("stroke-width", k % 3 === 0 ? wm : wf).attr("stroke-dasharray", k % 3 === 0 ? "none" : "2 4");
    }
  }
  ...
  function chartDonutLeadersSub(container, rows) {
    ...
    var gridDon = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    s2PolarFineRays(gridDon, cx, cy, or + 28, 36);
    d3.range(1, 16).forEach(function (i) {
      var rr = (i / 16) * (or + 24);
      gridDon.append("circle").attr("cx", cx).attr("cy", cy).attr("r", rr).attr("fill", "none")
        .attr("stroke", i % 4 === 0 ? CHART_GRID_STROKE : CHART_GRID_FINE).attr("stroke-width", i % 4 === 0 ? CHART_GRID_W : CHART_GRID_FINE_W).attr("stroke-dasharray", i % 4 === 0 ? "5 4" : "2 6");
    });
    ...
    var nbw = rows.filter(function (r) { return String(r[F.黑白]).indexOf("黑白") >= 0; }).length;
    g.append("text").attr("text-anchor", "middle").attr("y", -5).attr("fill", "#e8e8f0").style("font-size", "9px").text("n=" + rows.length);
    g.append("text").attr("text-anchor", "middle").attr("y", 9).attr("fill", "#9ab").style("font-size", "8px").text("黑白 " + Math.round(nbw / nt * 100) + "% · 彩 " + Math.round((nt - nbw) / nt * 100) + "%");
    setFigCaptionLegend(container, agg.map(function (d, i) { return { hex: PAL[i % 4], text: d.key }; }),
      "环形+外引线+密射线/同心细格；环心 n 与黑白/彩色占比（多维摘要）。样式对齐参考1 环形稿。点击扇区筛细分阶段。\n" + captionSampleLine(rows));
  }
  ...
  function chartPolarSpikeComplexity(container, rows) {
    ...
    var gr = 0;
    d3.range(0, 21).forEach(function (i) {
      var rr = (i / 20) * R;
      var c = gridG.append("circle").attr("cx", cx).attr("cy", cy).attr("r", rr).attr("fill", "none")
        .attr("stroke", i % 5 === 0 ? CHART_GRID_STROKE : CHART_GRID_FINE).attr("stroke-width", i % 5 === 0 ? CHART_GRID_W : CHART_GRID_FINE_W).attr("stroke-dasharray", i % 5 === 0 ? "none" : "2 5");
      revealCircleStroke(d3.select(c.node()), rr, 420, gr);
      gr += 5;
    });
    s2PolarFineRays(gridG, cx, cy, R, 24);
    var ptsW = keys.map(function (k) {
      var rs = rows.filter(function (r) { return String(r[F.复杂度]) === k; });
      var avg = rs.length ? d3.mean(rs, function (r) { return +r[F.字数] || 0; }) : 0;
      return { k: k, v: avg };
    });
    var maxW = d3.max(ptsW, function (d) { return d.v; }) || 1;
    var rW = d3.scaleLinear().domain([0, maxW]).range([18, R * 0.52]);
    var segW = ptsW.map(function (d, i) {
      var ang = (i / n) * 2 * Math.PI - Math.PI / 2;
      var rr = rW(d.v);
      return [cx + Math.cos(ang) * rr, cy + Math.sin(ang) * rr];
    });
    var lineW = d3.line().curve(d3.curveLinearClosed);
    ...
    svg.append("path").datum(segW).attr("d", lineW).attr("fill", "rgba(89,226,253,0.1)").attr("stroke", PAL[1]).attr("stroke-width", 2).attr("stroke-dasharray", "5 4").attr("opacity", 0.92);
    ...
    setFigCaptionLegend(container, [{ hex: PAL[0], text: "粉：条数包络" }, { hex: PAL[1], text: "青虚：平均字数" }],
      "双轨极坐标：外圈半径=各级条数；内圈虚线=该级平均字数。密同心圆+射线格（参考1）。\n" + captionSampleLine(rows));
  }
  ...
  function chartBeeswarmSubPackWc(container, rows) {
    ...
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var defsBee = svg.append("defs");
    var gridBee = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    ...
    subs.forEach(function (sub, ri) {
      ...
      s2FineGridXY(gridBee, m.l, yy + 4, m.l + iw, yy + rowH - 4, 28, 12);
      ...
      subRows.forEach(function (r, j) {
        ...
        var href = thumb(r);
        var clipId = "s2b_" + ri + "_" + j + "_" + (j * 17 + ri * 31);
        defsBee.append("clipPath").attr("id", clipId).append("circle").attr("cx", cxi).attr("cy", cyi).attr("r", Math.max(2.5, rad - 0.5));
        if (href) {
          var ig = svg.append("g").style("cursor", "pointer")
            .on("click", function () { setPhase(sub); setFilter(F.包装, String(pk)); })
            .on("mouseenter", function (ev) {
              tipShow(ev, "<img src=\"" + esc(href) + "\" alt=\"\">" + esc(norm(r[F.图字]) || "（无图内字）"));
            })
            .on("mousemove", moveTip).on("mouseleave", tipHide);
          ig.append("circle").attr("cx", cxi).attr("cy", cyi).attr("r", 0).attr("fill", col).attr("fill-opacity", 0.25).attr("stroke", "#fff").attr("stroke-width", 1.2)
            .transition().delay(j * 3).duration(300).ease(d3.easeElasticOut).attr("r", rad + 1.5);
          ig.append("image").attr("href", href).attr("x", cxi - rad).attr("y", cyi - rad).attr("width", rad * 2).attr("height", rad * 2)
            .attr("clip-path", "url(#" + clipId + ")").attr("preserveAspectRatio", "xMidYMid slice").attr("opacity", 0)
            .transition().delay(j * 3).duration(320).attr("opacity", 1);
        } else {
          svg.append("circle").attr("cx", cxi).attr("cy", cyi).attr("r", 0)
            ...
            .on("click", function () { setPhase(sub); setFilter(F.包装, String(pk)); })
            .on("mouseenter", function (ev) {
              tipShow(ev, esc(norm(r[F.图字]) || "（无图内字）"));
            })
            .on("mousemove", moveTip).on("mouseleave", tipHide)
    ...
    setFigCaptionLegend(container, [{ hex: PAL[0], text: "细分×包装×字数×情绪×缩略图" }],
      "密直角坐标网；有图址则绘圆形裁切缩略图，否则色点。悬停大图+图内字（参考1 气泡带）。\n" + captionSampleLine(rows));
  }
  ...
  function chartSplineDropEmotion(container, rows) {
    ...
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var gridG = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    s2FineGridXY(gridG, margin.l, margin.t, margin.l + iw, margin.t + ih, 22, 14);
    var gd = 0;
    ...
    var meanPack = top.map(function (d) {
      var rs = rows.filter(function (r) { return norm(r[F.情绪]) === norm(d.key); });
      return { key: d.key, m: rs.length ? d3.mean(rs, function (r) { return +r[F.包装] || 0; }) : 0 };
    });
    var maxM = d3.max(meanPack, function (d) { return d.m; }) || 1;
    var yM = d3.scaleLinear().domain([0, maxM]).nice().range([margin.t + ih, margin.t]);
    var lineM = d3.line().x(function (d) { return xS(d.key); }).y(function (d) { return yM(d.m); }).curve(d3.curveMonotoneX);
    svg.append("path").datum(meanPack).attr("fill", "none").attr("stroke", PAL[2]).attr("stroke-width", 2).attr("stroke-dasharray", "3 4").attr("d", lineM).attr("opacity", 0.85);
    meanPack.forEach(function (d, i) {
      svg.append("circle").attr("cx", xS(d.key)).attr("cy", yM(d.m)).attr("r", 4).attr("fill", PAL[2]).attr("fill-opacity", 0.5).attr("stroke", "#fff").attr("stroke-width", 1);
    });
    ...
    setFigCaptionLegend(container, top.map(function (d, i) { return { hex: PAL[i % 4], text: String(d.key).slice(0, 10) }; }).concat([{ hex: PAL[2], text: "紫虚：平均包装度" }]),
      "密网底+主样条（条数）+紫虚线（该情绪下平均包装度）+垂锚。参考1 线图层次。\n" + captionSampleLine(rows));
  }
  ...
  function chartHBarPercentScene(container, rows) {
    ...
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var defs = svg.append("defs");
    var gridH = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    d3.range(1, 24).forEach(function (i) {
      var xx = margin.l + (i / 24) * iw;
      gridH.append("line").attr("x1", xx).attr("x2", xx).attr("y1", margin.t).attr("y2", h - margin.b)
        .attr("stroke", i % 6 === 0 ? CHART_GRID_STROKE : CHART_GRID_FINE).attr("stroke-width", i % 6 === 0 ? CHART_GRID_W : CHART_GRID_FINE_W).attr("stroke-dasharray", i % 6 === 0 ? "none" : "2 5");
    });
  ...
  function chartDualAreaBWSub(container, rows) {
    ...
    var band = xS.step();
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var gridDA = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    s2FineGridXY(gridDA, margin.l, margin.t, margin.l + iw, margin.t + ih, 18, 14);
  ...
  function chartPictoBarsAction(container, rows) {
    ...
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var defs = svg.append("defs");
    var gridPic = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    s2FineGridXY(gridPic, margin.l, margin.t, margin.l + iw, margin.t + ih, 20, 16);
    var glyphs = ["●", "■", "◆", "▲", "▼", "◇"];
    agg.forEach(function (d, i) {
      var x = xS(d.key);
      var bw = xS.bandwidth();
  ...
  function chartRidgePackBySub(container, rows) {
    ...
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var gridR = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    s2FineGridXY(gridR, m.l, m.t, m.l + iw, m.t + subs.length * laneH, 26, subs.length * 6 + 4);
    svg.append("text").attr("x", m.l + iw / 2).attr("y", 14).attr("text-anchor", "middle").attr("fill", "#888").style("font-size", "9px").text("包装度 1—5 · 各细分阶段分布脊线 · 黄点=该档平均视觉复杂度");
    subs.forEach(function (sub, ri) {
      var yy = m.t + ri * laneH;
      var slice = rows.filter(function (r) { return norm(r[F.细分]) === sub; });
      ...
      packs.forEach(function (p, pj) {
        var cell = slice.filter(function (r) { return String(r[F.包装]) === p; });
        if (!cell.length) return;
        var mc = d3.mean(cell, function (r) { return +r[F.复杂度] || 0; });
        var cxb = xS(p) + xS.bandwidth() / 2;
        var ydot = y0 - (Math.min(5, Math.max(1, mc)) / 5) * (laneH - 16);
        svg.append("circle").attr("cx", cxb).attr("cy", ydot).attr("r", 3.5).attr("fill", PAL[3]).attr("stroke", "#111").attr("stroke-width", 0.8).attr("opacity", 0.9);
      });
    ...
    var gAx = svg.append("g").attr("transform", "translate(0," + (h - m.b + 6) + ")")
      ...
      .call(function (gg) { gg.select(".domain").attr("stroke", CHART_AXIS_DOMAIN_STROKE); });
    gAx.selectAll("text").attr("fill", "#999").style("font-size", "9px");
    setFigCaptionLegend(container, [{ hex: PAL[0], text: "脊线：条数" }, { hex: PAL[3], text: "黄点：该格平均复杂度" }],
      "密网底+脊线面积+每档黄点标平均视觉复杂度（细分×包装×复杂度）。点击脊线筛阶段。\n" + captionSampleLine(rows));
  }
  ...
  function chartConcentricSweepPack(container, rows) {
    ...
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var gridC = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    d3.range(1, 30).forEach(function (i) {
      var rr = (i / 30) * (r0 + packs.length * ring + 8);
      gridC.append("circle").attr("cx", cx).attr("cy", cy).attr("r", rr).attr("fill", "none")
        .attr("stroke", i % 5 === 0 ? CHART_GRID_STROKE : CHART_GRID_FINE).attr("stroke-width", i % 5 === 0 ? CHART_GRID_W : CHART_GRID_FINE_W).attr("stroke-dasharray", i % 5 === 0 ? "6 4" : "2 5");
    });
    s2PolarFineRays(gridC, cx, cy, r0 + packs.length * ring + 6, 32);
  ...
  function chartChordLiteOrigPack(container, rows) {
    ...
    var h = 380;
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var gridCh = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    s2FineGridXY(gridCh, 24, 36, w - 24, h - 28, 28, 18);
    var leftX = 56;
    var rightX = w - 56;
  ...
  function chartMatrixBubbleDirectPack(container, rows) {
    ...
    var svg = d3.select(container).append("svg").attr("class", "chart-svg chart-pixel-svg").attr("width", w).attr("height", h);
    var defsMx = svg.append("defs");
    var gridMx = svg.append("g").attr("class", "chart-grid-layer").attr("pointer-events", "none");
    s2FineGridXY(gridMx, m.l, m.t, m.l + packs.length * cellW, m.t + dirs.length * cellH, packs.length * 6, dirs.length * 5);
    ...
    dirs.forEach(function (dir, i) {
      ...
      packs.forEach(function (p, j) {
        ...
        var rUse = Math.min(rad, cellW * 0.38);
        var href = slice[0] ? thumb(slice[0]) : "";
        var clipM = "s2mx_" + i + "_" + j;
        defsMx.append("clipPath").attr("id", clipM).append("circle").attr("cx", cx).attr("cy", cy).attr("r", rUse);
        var gCell = svg.append("g").style("cursor", "pointer")
          .on("click", function () { setFilter(F.直接, dir); setFilter(F.包装, p); })
          .on("mouseenter", function (ev) {
            var rk = aggregate(slice, F.风险)[0];
            var t = esc(String(dom ? dom.key : "")) + "<br>" + n + " 条 · 风险 " + esc(rk ? rk.key : "—");
            tipShow(ev, (href ? "<img src=\"" + esc(href) + "\" alt=\"\">" : "") + t);
          })
          .on("mousemove", moveTip).on("mouseleave", tipHide);
        gCell.append("circle").attr("cx", cx).attr("cy", cy).attr("r", 0).attr("fill", col).attr("fill-opacity", 0.35).attr("stroke", "#fff").attr("stroke-width", 1.2)
          .transition().duration(400).delay((i * packs.length + j) * 18).attr("r", rUse + 1);
        if (href) {
          gCell.append("image").attr("href", href).attr("x", cx - rUse).attr("y", cy - rUse).attr("width", rUse * 2).attr("height", rUse * 2)
            .attr("clip-path", "url(#" + clipM + ")").attr("preserveAspectRatio", "xMidYMid slice").attr("opacity", 0)
            .transition().duration(400).delay((i * packs.length + j) * 18).attr("opacity", 1);
        } else {
          gCell.append("circle").attr("cx", cx).attr("cy", cy).attr("r", 0).attr("fill", col).attr("fill-opacity", 0.75).attr("stroke", "#fff").attr("stroke-width", 1.2)
            .transition().duration(400).delay((i * packs.length + j) * 18).attr("r", rUse);
        }
      });
    });
    setFigCaptionLegend(container, [{ hex: PAL[0], text: "直接×包装×缩略图" }],
      "密格网+裁切缩略图+色环；悬停主情绪与主导风险。多维交叉（参考1）。\n" + captionSampleLine(rows));
  }
